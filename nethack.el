...
(defcustom nethack-use-tiles nil
  ...
  :set (lambda (var val)
         (set-default var (if (eq val t) "nethack" val)))
  :group 'nethack)
...
(defun nethack-toggle-tiles ()
  ...
  (if nethack-use-tiles
    ...
    (setq nethack-use-tiles nethack-use-tiles--tileset))
...
\\{nethack-map-mode-map}"
  ...
  (when (and nethack-use-tiles (display-graphic-p))
    (setq nethack-use-tiles--font-height (face-remap-add-relative 'default :height 16))
